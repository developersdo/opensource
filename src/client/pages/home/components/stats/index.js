        ...
        ${ langs.map((lang) => {
          return html`
            <div>
              ...
              (${ lang.total.toLocaleString() } repos)
      ...
      <div>
        ...
        <ul class="collection">
          <li class="collection-item">
            <h4><i class="material-icons">perm_identity</i> ${ state.users.length.toLocaleString() } users <small>are contributing to open source.</small></h4>
            <p>(organizations are not counted)</p>
            <p>Among that number of users, <strong>${ usersWithMoreThanTenRepo.toLocaleString() } have more 10 repos</strong>.</p>
          </li>
          <li class="collection-item">
            <h4><i class="material-icons">code</i> ${ state.repos.length.toLocaleString() } repos <small>created on GitHub. In average, each user contribute with ${ (state.repos.length / state.users.length).toFixed(1).toLocaleString() } repos.</small></h4>
            <p>(excluding forks)</p>
          ...
          <li class="collection-item">
            <h4><i class="material-icons">assessment</i> ${ langs.length.toLocaleString() } programming languages <small>in use across ${ state.repos.length.toLocaleString() } repos.</small></h4>
